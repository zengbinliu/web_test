...
>
> 整理日期：2026-08-05（十三章对照按 2026-08 askreolink 实现补充；2026-08-13 补全 ID 段 / MCP / 入库脚本；2026-08-16 补充 TF-IDF / 稀疏检索）
...
1. [RAG 是什么](#一rag-是什么)
2. [整体架构](#二rag-整体架构)
3. [核心概念](#三核心概念逐个讲)（含 [TF-IDF](#38-tf-idf词频-逆文档频率)、[稀疏检索](#39-稀疏检索)）
4. [完整问答流程](#四完整问答流程)
...
- **轻量方案**：hash + TF-IDF 风格稀疏向量（见 [3.8](#38-tf-idf词频-逆文档频率)、[3.9](#39-稀疏检索)；askreolink 走这条路）
...
- **稠密检索**：Embedding 向量，看语义相近（见 3.3）
- **稀疏检索**：词本身匹配 + TF-IDF / BM25 加权（见 [3.8](#38-tf-idf词频-逆文档频率)、[3.9](#39-稀疏检索)）
...
**TF-IDF**（Term Frequency–Inverse Document Frequency）衡量「某个词对某篇文档有多重要」，是稀疏检索、关键词提取、早期搜索引擎打分的基础。
...
**稀疏检索**（Sparse Retrieval）用「词本身」做匹配：文档和查询都被表示成**很长、但大部分位置为 0** 的向量，只有出现过的词才有非零权重。
...
只靠稀疏检索，容易漏掉同义表达；只靠稠密检索，容易漏掉冷门专有词。两者互补。
...
把问题和 chunk 都编成 Embedding，按余弦相似度取 Top-K。语义相近就能命中，但对**精确 ID、编号、套餐名**可能不够准。原理见 [3.3](#33-向量化embedding)、[3.9](#39-稀疏检索) 对照表。

### 6.2 关键词检索（BM25 / 稀疏向量）

本质是 [3.8 TF-IDF](#38-tf-idf词频-逆文档频率) / [3.9 稀疏检索](#39-稀疏检索)：按词是否出现、词有多稀有来打分。
...
- [ ] 说清 TF-IDF（本地常见 × 全局稀有）和稀疏检索 vs 稠密检索
