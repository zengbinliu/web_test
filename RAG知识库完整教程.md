...
>
> 整理日期：2026-08-05（十三章对照按 2026-08 askreolink 实现补充；2026-08-13 补全 ID 段 / MCP / 入库脚本；2026-08-16 补充 TF-IDF / 稀疏检索；2026-08-17 补充稠密/稀疏向量嵌入）
...
1. [RAG 是什么](#一rag-是什么)
2. [整体架构](#二rag-整体架构)
3. [核心概念](#三核心概念逐个讲)（含 [TF-IDF](#38-tf-idf词频-逆文档频率)、[稀疏检索](#39-稀疏检索)、[稠密/稀疏嵌入](#310-稠密向量嵌入与稀疏向量嵌入)）
4. [完整问答流程](#四完整问答流程)
...
把文本变成**数字向量**，方便用距离/相似度做检索。向量有两大类（详见 [3.10](#310-稠密向量嵌入与稀疏向量嵌入)）：

- **稠密嵌入**：固定短向量（如 384 / 768 / 1536 维），几乎每个维度都有值，捕捉**语义**
- **稀疏嵌入**：极长向量（词表大小或 hash 桶），绝大部分为 0，捕捉**关键词**
...
- **云端 API（稠密）**：OpenAI `text-embedding-3-small`
- **本地模型（稠密）**：`sentence-transformers` 多语言模型（BGE、E5 等）
- **轻量方案（稀疏）**：hash + TF-IDF 风格稀疏向量（见 [3.8](#38-tf-idf词频-逆文档频率)、[3.9](#39-稀疏检索)、[3.10](#310-稠密向量嵌入与稀疏向量嵌入)；askreolink 走这条路）
...
检索不只有「语义向量」一种。按表示方式可分成：

- **稠密检索**：稠密嵌入，看语义相近（见 [3.3](#33-向量化embedding)、[3.10](#310-稠密向量嵌入与稀疏向量嵌入)）
- **稀疏检索**：稀疏嵌入 / 词本身匹配 + TF-IDF / BM25 加权（见 [3.8](#38-tf-idf词频-逆文档频率)、[3.9](#39-稀疏检索)、[3.10](#310-稠密向量嵌入与稀疏向量嵌入)）
- **混合检索**：两者融合（见 [第六章](#六检索策略进阶)）
...
**TF-IDF**（Term Frequency–Inverse Document Frequency）（打分公式）衡量「某个词对某篇文档有多重要」，是稀疏检索、关键词提取、早期搜索引擎打分的基础。稀疏检索里，用 BM25（或 TF-IDF）来打分

一句话：**在本文里常见，但在全集里不常见** 的词，得分最高。

#### 两个部分

**TF（词频）**：这个词在这篇文档里出现得有多勤。
...
**IDF（逆文档频率）**：这个词在整个语料库里有多「稀有」。

```
IDF(t) = log( 文档总数 / 包含词 t 的文档数 )
```

几乎每篇都出现的词（如「的」「是」）IDF 很小；只在少数文档出现的词 IDF 很大。
...
| 词 | 在「摄像头说明书」里 | 在多少篇文档里出现 | 结果 |
|----|----------------------|--------------------|------|
| 「的」 | 很多次 | 几乎全部 | TF 高、IDF 极低 → 不重要 |
| 「云存储」 | 几次 | 只有少数文档 | TF 中等、IDF 高 → 很能代表这篇 |

所以 TF-IDF 会把「云存储」抬高，把停用词压下去。测试场景里，「Argus 4 Pro」「/v2/shop/orders」「仅带图」这类专有词，IDF 通常很高，适合精确命中。

#### 常见用途

- 搜索引擎：查询词与文档的匹配打分
- 关键词提取：取一篇文档里 TF-IDF 最高的若干词
- 文本向量化：把文档变成词权重向量，再做相似度、聚类、分类
- RAG / 知识库检索：作为**稀疏检索**的打分基础（和 BM25 同类思路）

#### 和 BM25 的关系

**BM25** 可以看成 TF-IDF 的改进版，多了以下东西：

- 对词频做**饱和**：出现 100 次不会比 10 次重要 10 倍
- 考虑**文档长度**：长文档不会仅仅因为词更多就得分更高

现在检索系统更常用 BM25，但核心思想仍是「本地常见 × 全局稀有」。第六章的「关键词检索」指的就是这一路。

##### 什么时候用哪个

- 要抽关键词、做特征、讲原理 → TF-IDF 足够，也好讲。
- 要做搜索 / RAG 稀疏侧 → 用 BM25，别自己用原始 TF-IDF 当主排序。
- 要换说法也能搜到 → 稀疏（BM25）不够，加稠密嵌入，做混合检索。

### 3.9 稀疏检索

**稀疏检索**（Sparse Retrieval）用「词本身」做匹配：文档和查询都被表示成**很长、但大部分位置为 0** 的向量，只有出现过的词才有非零权重。（是检索方式）

「稀疏」指的是向量里绝大多数维度是空的，不是说结果少。

#### 和稠密检索对比

| | 稀疏检索 | 稠密检索 |
|---|----------|----------|
| 表示 | 词表维度（几万～百万），大部分为 0 | 几百维连续向量，几乎全非零 |
| 匹配依据 | 关键词是否出现、词频、IDF | 语义是否相近 |
| 典型算法 | TF-IDF、BM25、SPLADE | Embedding + 向量相似度（如 BERT、BGE） |
| 擅长 | 专有名词、型号、精确术语、case_id | 同义改写、语义相近但用词不同 |
| 不擅长 | 「怎么备份录像」vs「如何保存回放」这种换说法 | 精确型号、罕见专有词可能对不齐 |

例如查询「Argus 4 Pro 云套餐」：

- 稀疏检索会直接盯住 `Argus`、`4`、`Pro`、`云套餐` 这些词
- 稠密检索更可能把「电池摄像机订阅」也找出来，即使原文没写「Argus 4 Pro」

#### 为什么叫稀疏

假设词表有 10 万个词，一篇文档只出现了 80 个不同的词，那这条向量就是：

- 长度 100,000
- 只有约 80 个位置有 TF-IDF / BM25 分数
- 其余全是 0

所以叫稀疏向量。检索时通常不真的存 10 万维数组，而是用**倒排索引**：先找「包含这些词的文档」，再按分数排序。

askreolink 的 `text_to_vector()` 是另一条轻量路：不建完整词表，而是把词 **hash 到 8192 个桶**，桶内用 `1 + log(count)` 加权再 L2 归一化——本质仍是稀疏/类 TF-IDF，只是维度被压到固定 8192。

#### 工作流程（BM25 最常见）

```
1. 把文档分词、建倒排索引（每个词 → 出现在哪些文档）
2. 用户查询同样分词
3. 取出包含这些词的候选文档
4. 用 TF-IDF / BM25 打分排序
5. 返回 Top-K
```

核心是 **字面匹配 + 词重要性加权**，不是理解句子意思。
...
很多系统会做 **混合检索**（见 [6.3](#63-混合检索hybrid-search生产常用)）：

- 稀疏检索：保证「型号、接口名、错误码、case_id」能精确命中
- 稠密检索：保证「换一种说法」也能找到相关段落
- 再用融合（加权求和或 RRF）或重排序把两边结果合并

只靠稀疏检索，容易漏掉同义表达；只靠稠密检索，容易漏掉冷门专有词。两者互补。嵌入本身怎么表示，见下一节。

### 3.10 稠密向量嵌入与稀疏向量嵌入

**嵌入（Embedding）**是「把文本变成向量」；**检索**是「用这些向量去找相似文档」。3.9 讲检索怎么用稀疏向量，本节讲两种嵌入**本身**是什么、差在哪、什么时候用。
...
把一段文本压成一个**固定长度、几乎每个维度都有非零值**的向量，例如 384、768、1536 维。

- 每个维度通常没有人类可读含义，是模型学出来的「语义方向」
- 语义相近的内容，向量在空间里距离更近
- 「猫坐在垫子上」和「一只小猫趴在地毯上」字面不同，但向量会很接近
- 测试场景：「摄像头连不上 WiFi」和「设备无法加入无线网络」也会比较近

典型模型：BERT、OpenAI `text-embedding-3`、BGE、E5。检索时用近似最近邻（HNSW、IVF、Faiss 等）。

#### 稀疏向量嵌入（Sparse Embedding）

向量维度非常大（可到词表大小，几万到几十万维），但**绝大多数维度是 0**，只有少数维度有值。

- 每个非零维度通常对应一个词、子词，或模型学出来的词项
- 更接近传统关键词检索：哪个词出现了、有多重要
- 「Reolink 云套餐续费」会在「Reolink」「云套餐」「续费」这些位置有权重，其余几乎为 0

典型方法：BM25（经典稀疏）、SPLADE、BGE-M3 的稀疏头、Learned Sparse Retrieval。askreolink 的 `text_to_vector()` 是 hash 到 8192 维的轻量稀疏嵌入。

检索时通常不真的存整条超长数组，而是用**倒排索引**：先找包含这些词的文档，再按权重排序。

#### 区别对照

| 维度 | 稠密嵌入 | 稀疏嵌入 |
|------|----------|----------|
| 向量形态 | 短、几乎全非零 | 极长、绝大部分为 0 |
| 捕捉什么 | 语义、同义、改写 | 精确词、专有名词、数字、罕见词 |
| 可解释性 | 差，维度难解释 | 较好，能对应到词项 |
| 检索方式 | 近似最近邻（HNSW、IVF 等） | 倒排索引（和 BM25 同一套） |
| 对「换一种说法」 | 强 | 弱，除非词重叠 |
| 对「必须命中这个词」 | 弱，容易语义漂移 | 强 |
| 存储 / 计算 | 每条都要存完整向量 | 只存非零项，索引成熟 |

和 3.9 那张「稀疏检索 vs 稠密检索」表是同一件事的两面：那边讲**怎么搜**，这边讲**文本先被编成什么样的向量**。

#### 什么时候用哪种

**更适合稠密**

- 用户问法和文档用词差很多（同义、口语、跨语言）
- 要做语义搜索、问答、相似文档、推荐
- 文档较短、主题明确，关键词不一定对得上
- 例子：用户搜「摄像头连不上 WiFi」，文档写的是「设备无法加入无线网络」

**更适合稀疏**

- 必须精确命中：型号、订单号、错误码、人名、SKU、API 名、`case_id`
- 领域术语、低频专有词（稠密模型容易把它们「抹平」）
- 要可解释：为什么命中，因为命中了哪些词
- 语料很大、要沿用倒排索引、延迟和成本更可控
- 例子：搜 `RLC-810A`、`errcode=401`、`askreolink`，不能搜成别的型号或别的接口

**经验规则**

- 只有语义、不怕漏专有词 → 稠密即可
- 只有精确匹配、不怕换说法漏召回 → 稀疏即可
- 既要「换说法也能找到」，又要「型号 / ID 必须准」→ **混合检索**（见 [6.3](#63-混合检索hybrid-search生产常用)）

#### 生产上两者一起用

多数检索系统会做 **Hybrid Search**：

1. 稀疏（BM25 / SPLADE / hash 稀疏向量）保证关键词不丢
2. 稠密保证语义能召回
3. 再用加权分数或 RRF 融合，必要时加 rerank

对 Reolink 这类知识库（需求、禅道用例、产品文档）：

- 稠密负责「用户怎么问都能对上意思」
- 稀疏负责「型号、模块名、用例编号不能搜偏」

askreolink 当前是 **关键词字段加权 + hash 稀疏向量** 的混合，没有上神经网络稠密模型。要补语义泛化，才需要再加一层 BGE / OpenAI embedding，而不是替换掉稀疏侧。
...
把问题和 chunk 都编成 Embedding，按余弦相似度取 Top-K。语义相近就能命中，但对**精确 ID、编号、套餐名**可能不够准。原理见 [3.3](#33-向量化embedding)、[3.10](#310-稠密向量嵌入与稀疏向量嵌入)，对照表见 [3.9](#39-稀疏检索)。

### 6.2 关键词检索（BM25 / 稀疏向量）

本质是 [3.8 TF-IDF](#38-tf-idf词频-逆文档频率) / [3.9 稀疏检索](#39-稀疏检索) / [3.10 稀疏嵌入](#310-稠密向量嵌入与稀疏向量嵌入)：按词是否出现、词有多稀有来打分。

擅长精确匹配：`case_id=12345`、`云套餐-仅带图`、`/v2/shop/orders`。

常见实现：

| 实现 | 特点 | 本教程对照 |
|------|------|------------|
| **BM25** | TF-IDF 改进版，词频饱和 + 文档长度归一 | 通用搜索、Elasticsearch 默认 |
| **倒排索引 + TF-IDF** | 先按词召回，再算权重 | 经典稀疏检索 |
| **hash 稀疏向量** | 不建词表，词 hash 到固定维度 | askreolink `text_to_vector()`，`dim=8192` |
| **字段加权打分** | 标题/模块/步骤/预期不同权重 | askreolink `score_case()` |

生产里「关键词检索」往往不是单一公式，而是 **BM25 / 稀疏向量 + 业务字段加权**。

### 6.3 混合检索（Hybrid Search，生产常用）

稀疏保证专有词命中，稠密（或 hash 向量）补语义，再融合：

```
最终得分 = α × 向量相似度 + (1-α) × 关键词得分
```

也可不用加权，改用 **RRF（Reciprocal Rank Fusion）**：按两边排名倒数求和，少调 α。

askreolink 实现：`keyword 0.55` + `vector 0.45`（见 [13.6](#136-query一次-askreolink-问答流水线)）。
...
- [ ] 说清 TF-IDF（本地常见 × 全局稀有）、稠密/稀疏嵌入，以及稀疏检索 vs 稠密检索
...
| Embedding | **不是** sentence-transformers；是 **hash 稀疏向量**（类 TF-IDF，见 [3.8](#38-tf-idf词频-逆文档频率)、[3.9](#39-稀疏检索)） | `text_to_vector()`，`dim=8192` |
...
纯向量对「云套餐-仅带图」「case_id=12345」往往不如关键词（TF-IDF / BM25 / 稀疏检索，见 [3.8](#38-tf-idf词频-逆文档频率)、[3.9](#39-稀疏检索)）。
